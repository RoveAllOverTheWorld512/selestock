...
def del_gpdm(days):
    ...
    m1=(td+datetime.timedelta(-days)).strftime("%Y-%m-%d")
...
def yburldic():
    ...
    dbcn = sqlite3.connect(dbfn)
    curs = dbcn.cursor()
    sql='select gpdm,yburl from cfi_yburl where ok=1;'
    curs.execute(sql)        
    data = curs.fetchall()
...
#从中财网读取研报数据
###############################################################################
def get_cfiyb(dbcn,gpdm,urldic):
    pjjgdic=jgdic()
    ...
    if not gpdm in urldic.keys():
        ...
        try :            
            ...
            elem=browser.find_element_by_xpath("//a[text()='研报一览']")
