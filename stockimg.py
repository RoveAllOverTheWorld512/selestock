...
def get_gdhs(gpdm):
    ...
    curs = dbcn.cursor()
    sql = 'SELECT rq,gdhs FROM DFCFGDHS WHERE GPDM="%s" ORDER BY RQ;' % gpdm
    curs.execute(sql)
...
def get_lngbbd_gao(gpdm):
    ...
    dbcn = sqlite3.connect(dbfn)
    curs = dbcn.cursor()
    sql = 'SELECT rq,zgb,ltgb,sjltgb FROM lngbbd WHERE GPDM="%s" ORDER BY RQ;' % gpdm
    curs.execute(sql)
...
def get_pepb(gpdm):
    ...
    dbcn = sqlite3.connect(dbfn)
    curs = dbcn.cursor()
    sql = 'SELECT RQ,PE_LYR,PE_TTM,PB FROM PE_PB WHERE GPDM="%s" ORDER BY RQ;' % gpdm
    curs.execute(sql)
...
def get_shcg(gpdm):
    ...
    dbcn = sqlite3.connect(dbfn)
    curs = dbcn.cursor()
    sql = 'SELECT RQ,SHCG FROM LTG WHERE GPDM="%s" ORDER BY RQ;' % gpdm
    curs.execute(sql)
...
#    lxlb='"5"'
    ...
    fqgj=get_fqgj(gpdm,['volume','close'])
    
        
    if fqgj.index[0].to_pydatetime().strftime('%Y-%m-%d')>start:
        start=fqgj.index[0].to_pydatetime().strftime('%Y-%m-%d')

    shcg=get_shcg(gpdm)    

    gdhs=get_gdhs(gpdm)   
    if len(gdhs)==0:
        gdhs=get_gdhs_dzh(gpdm)
        
    #信息合并    
    hjcg=shcg.join(fqgj,how='outer')
    hjcg=hjcg.join(gdhs,how='outer')

    hjcg=hjcg.sort_index()
    
    #缺失数据
    hjcg['close'].fillna(method='ffill',inplace=True)
    hjcg['gdhs'].fillna(method='ffill',inplace=True)
    hjcg['shcg'].fillna(method='ffill',inplace=True)
    
    #计算户均持股与市值
    hjcg=hjcg.assign(hjcg=hjcg['shcg']/(hjcg['gdhs']-10))
    hjcg=hjcg.assign(hjsz=hjcg['hjcg']*hjcg['close'])

        
    pepb=get_pepb(gpdm)
    
    gbbd=get_lngbbd_gao(gpdm)
...
#    data = data.set_index('date')
    ...
    fqgj = fqgj.loc[start:end,:]
    gdhs = gdhs.loc[start:end,:]

    hjcg = hjcg.loc[start:end,:]

    hs300=hs300.loc[start:end,:]
    dbzs=dbzs.loc[start:end,:]
    pepb=pepb.loc[start:end,:]

    gbbd=gbbd.join(hs300,how='outer')
    gbbd.fillna(method='ffill',inplace=True)
    gbbd=gbbd.loc[start:end,:]

    ymin=fqgj['adj_close'].describe()['min']*0.8
    ymax=fqgj['adj_close'].describe()['max']*1.2

    xmin=plt.matplotlib.dates.date2num(dateutil.parser.parse(start))
    xmax=plt.matplotlib.dates.date2num(dateutil.parser.parse(end))
    ...
    ax1 = plt.subplot2grid((18, 1), (0, 0), rowspan=7)    
    ax5 = ax1.twinx()

    ax2 = plt.subplot2grid((18, 1), (7, 0))

    ax6 = plt.subplot2grid((18, 1), (8, 0), rowspan=2)
    ax7 = ax6.twinx()        
    
    ax10 = plt.subplot2grid((18, 1), (10, 0), rowspan=2)

    ax8 = plt.subplot2grid((18, 1), (12, 0), rowspan=3)
    ax9 = ax8.twinx()        
    
    ax3 = plt.subplot2grid((18, 1), (15, 0), rowspan=3)
    ax4 = ax3.twinx()        
...
#    ax3 = ax2.twinx()
    ...
    ax1.plot(fqgj.index,fqgj['adj_close'],color="red",linewidth=1.5,label='adj_close')
    ax2.bar(fqgj.index,fqgj['volume'],color="r",linewidth=1.5,label='成交量')
    ...
    ax5.plot(gdhs.index,gdhs['gdhs'],color="y",linewidth=1.5,label='股东户数')

    ax6.plot(pepb.index,pepb['pe_ttm'],color="r",linewidth=1.5,label='PE_TTM')
    ax6.plot(pepb.index,pepb['pe_lyr'],color="y",linewidth=1.5,label='PE_LYR')

    ax7.plot(pepb.index,pepb['pb'],color="b",linewidth=1.5,label='PB')
    
    ax8.plot(hjcg.index,hjcg['hjcg'],color="b",linewidth=1.5,label='户均持股')
    ax9.plot(hjcg.index,hjcg['hjsz'],color="r",linewidth=1.5,label='户均市值')

    ax10.bar(gbbd.index,gbbd['zgb'],color="r",linewidth=1.5,label='总股本')
    ax10.bar(gbbd.index,gbbd['ltgb'],color="b",linewidth=1.5,label='流通股本')
    ax10.bar(gbbd.index,gbbd['sjltgb'],color="g",linewidth=1.5,label='实际流通股本')
    ...
    pemax=min(200,pepb['pe_ttm'].describe()['max'],pepb['pe_lyr'].describe()['max'])
    ax6.set_ylim(0,pemax)
    
    pbmax=min(20,pepb['pb'].describe()['max'])
    ax7.set_ylim(0,pbmax)
    ...
    ax8.set_ylabel('户均持股(万股)',color="b", fontsize = 16)
    ax9.set_ylabel('户均市值(万元)',color="b", fontsize = 16)
    ax10.set_ylabel('股本(亿股)',color="b", fontsize = 16)
    ...
    ax6.grid(True,color='r',linestyle=':')
    ax7.grid(True,color='b',linestyle=':')
    ax8.grid(True,color='b',linestyle=':')
    ax9.grid(True,color='r',linestyle=':')
    ax10.grid(True,color='r',linestyle=':')
    ...
    tsxx =tsxx.loc[start:end,:]
    
    data1=fqgj.copy()
    dstx=data1.join(tsxx,how='outer')
    dstx['adj_close'].fillna(method='ffill',inplace=True)

    #必须指定这两列，不指定会把增补的成交量为空的也删除
    dstx=dstx.dropna(subset=['adj_close','dstx'])
    ...
    #将时间轴最大值转换为pandas的时间戳
#    xm=pd.Timestamp(dateutil.parser.parse(end))
    
    for rq in dstx.index:
        x0=dstx.loc[rq]['x0']
        x1=x0+datetime.timedelta(int(dstx.loc[rq]['l']*xfont12+30))
        #这段代码本来是想将提示信息控制在ax坐标系内,对于超出
#        if x1>xm:
#            qy=x1-xm      #前移量
#            x0=x0-qy
#            x1=x1-qy
