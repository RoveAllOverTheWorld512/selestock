...
########################################################################
def iniconfig():
    myname = os.path.splitext(os.path.realpath(__file__))[0]
    inifile = myname + '.ini'  # 设置缺省配置文件
...
if __name__ == '__main__':
    ...
    wkdir = os.path.dirname(os.path.realpath(__file__))  # 程序所在文件夹为工作目录

    try:
        opts, args = getopt.getopt(sys.argv[1:], 'hvc:b:e:', [
                                   'help', 'version', 'cfg=', 'bgdate=', 'eddate='])
    ...
    jyrqs = jyrqs[((jyrqs['cal_date'] >= bgdate) &
                   (jyrqs['cal_date'] <= eddate))]
