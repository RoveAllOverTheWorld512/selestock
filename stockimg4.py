...
#    data = data.set_index('date')
    ...
    #因为年报季报发布日期可能不是交易日，所以必须全连接补齐日期
    data=fqgj.join(gdhs,how='outer')

    #避免出现断线
    data['gdhs'].fillna(method='ffill',inplace=True)    
    data['adj_close'].fillna(method='ffill',inplace=True)
...
#    xmin,xmax=ax1.get_xlim()        
    ...
    for rq in dstx.index:
        x0=dstx.loc[rq]['x0']
        x1=x0+datetime.timedelta(int(dstx.loc[rq]['l']*xfont12+24))
#        if x1>endtime:
#            qy=x1-endtime      #前移量
#            x0=x0-qy
#            x1=x1-qy
